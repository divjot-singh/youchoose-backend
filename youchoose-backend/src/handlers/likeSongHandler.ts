...
export const UnlikeSong = async (req:Request<any, any,LikeSongBody, any>, res:Response, next:NextFunction) => {
    try{
        console.log('inside LikeUnlikeSong')
        const data:LikeSongBody = req.body
        const returnVal:void | Error = await FirebaseService.unLikeSong(data.userId, data.song)
        if(instanceOfError(returnVal)){
            res.status(200).send({success:false, error:returnVal})
        } else{
            res.status(200).send({success:true})
...
export const LikeSong = async (req:Request<any, any,LikeSongBody, any>, res:Response, next:NextFunction) => {
    try{
        console.log('inside LikeUnlikeSong')
        const data:LikeSongBody = req.body
        const returnVal:void | Error = await FirebaseService.likeSong(data.userId, data.song)
        if(instanceOfError(returnVal)){
            res.status(200).send({success:false, error:returnVal})
        } else{
            res.status(200).send({success:true})
